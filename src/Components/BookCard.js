const BookCard=({title,cover ,onClick})=>{
  ...
  //On click of card calls onClick prop to show popup
    return <div  onClick={onClick} className="w-56 gap-3 md:w-60 bg-indigo-200 rounded-md shadow-lg border-2 overflow-hidden cursor-pointer on hover:bg-indigo-400">
        <h3>{title}</h3>
    {cover ? (
